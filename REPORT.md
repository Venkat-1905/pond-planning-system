...
$$x = (\lambda - \lambda_0) \cdot rac{\pi}{180} \cdot R \cdot \cos(\phi_0)$$
$$y = (\phi - \phi_0) \cdot rac{\pi}{180} \cdot R$$
...
2. Generates an adaptive regular grid $(\Delta x = \Delta y pprox 5	ext{--}10	ext{ m})$.
...
$$rac{\partial z}{\partial x} pprox rac{Z_{i, j+1} - Z_{i, j-1}}{2 \Delta x}, \quad rac{\partial z}{\partial y} pprox rac{Z_{i+1, j} - Z_{i-1, j}}{2 \Delta y}$$
$$	ext{Slope}(\%) = \sqrt{\left(rac{\partial z}{\partial x}ight)^2 + \left(rac{\partial z}{\partial y}ight)^2} 	imes 100\%$$
...
$$S_k = rac{Z_{r, c} - Z_{r+\Delta r_k, c+\Delta c_k}}{d_k \cdot \Delta}$$
...
$$V_{	ext{runoff}} (	ext{m}^3) = rac{C 	imes R (	ext{mm}) 	imes A (	ext{m}^2)}{1000}$$
...
- **Target Storage & Trapezoidal Pond Geometry:**
  ...
  - Surface Area: $A_{	ext{top}} = rac{V_{	ext{target}}}{D_{	ext{water}} 	imes 0.85}$ assuming $1.5:1$ length-to-width aspect ratio.
...
**Request:**
```bash
curl -X POST "http://10.1.75.51:5233/analyzeContour"      -H "accept: application/json"      -F "file=@contours_1m.kml"
```
...
```bash
curl -X POST "http://10.1.75.51:5233/findCatchment"      -F "file=@contours_1m.kml"      -F "land_cover=agricultural"
```

**Request (Manual Coordinates Override):**
```bash
curl -X POST "http://10.1.75.51:5233/findCatchment"      -F "file=@contours_1m.kml"      -F "pond_lat=21.244266"      -F "pond_lon=81.288278"      -F "land_cover=agricultural"      -F "runoff_coeff=0.45"      -F "rainfall_mm=1150"
```
...
**Request:**
```bash
curl -X POST "http://10.1.75.51:5233/pond/analyze"      -H "Content-Type: application/json"      -d '{
       "pond_lat": 21.244266,
       "pond_lon": 81.288278,
